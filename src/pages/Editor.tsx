...
const Editor = () => {
  ...
  const cleanMessageForChat = (content: string): string => {
    ...
    cleanContent = cleanContent.replace(/<!DOCTYPE html>[\s\S]*?<\/html>/gi, '');
  ...
  const handleInitialPrompt = async (prompt: string) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: "error-" + Date.now(),
        type: "assistant",
        content: `Desculpe, ocorreu um erro: ${error instanceof Error ? error.message : 'Erro desconhecido'}. Verifique se a chave da API do OpenAI está configurada corretamente.`,
        timestamp: new Date()
      };
